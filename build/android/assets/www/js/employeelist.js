...
function getEmployees_success(tx, results) {
	...
	var len = 0;
	$.ajax({
		url: 'http://dev-appservicepresse.glasshouse.fr/list.php',
		dataType: 'jsonp',
		jsonp: 'jsoncallback',
		timeout: 5000,
		success: function(data, status){
			$.each(data, function(i,item){ 
				//$('#employeeList').append('<li>' + item.titre + '☆</li>');
				$('#employeeList').append('<li>' +
					'<div class="line1">' + item.titre + '</div>' +
					'<span class="bubble"> ☆ </span></li>');
				len += 1;
					
			});
		},
		error: function(){
			$("#employeeList").html("Erreur lors de la lecture des données !");
		}
	});
